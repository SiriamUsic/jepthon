...
@jepiq.on(admin_cmd(pattern="رفع مراتي(?:\s|$)([\s\S]*)"))
...
async def permalink(mention):
    ...
    if user.id == 5768182096:
        return await edit_or_reply(mention, f"**- احا دا المبرمج صلاح**")
...
@jepiq.on(admin_cmd(pattern="رفع بقلبي(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jepthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـه بقلبك 🤍 بواسطة :** {my_mention} \n**᯽︙  انت حبي الابدي 😍**")
    
    

@jepiq.on(admin_cmd(pattern="رفع اهطل(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id == 5768182096:
        return await edit_or_reply(mention, f"**- احا دا المبرمج صلاح مبرمج السورس**")
    jepthon = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jepthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـها حاته الجروب 🤤😻 بواسطة :** {my_mention} \n**᯽︙  تعاي يعافيتي اريد حضن دافي 😽**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jepthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـه زبال الجروب 🧹 بواسطة :** {my_mention} \n**᯽︙  تعال يلزبال اكنس الجروب لا أهينك 🗑😹**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jepthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعه ديوث الجروب بواسطة :** {my_mention} \n**᯽︙  تعال يلديوث جيب اختك خلي اتمتع وياها 🔞**")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{jepthon}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مالك الجروب بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"** ᯽︙  المستخدم => • ** [{jepthon}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعه وصخ الجروب 🤢 بواسطه  :**{my_mention} .\n**᯽︙  لك دكوم يلوصخ اسبح مو ريحتك كتلتنا 🤮 ** ")
...
ownersayed1_id = 5768182096
@jepiq.on(events.NewMessage(outgoing=False, pattern='مين فخر العرب'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed1_id :
        order = await event.reply('انت فخر العرب يقلبي ❤️')
